...
flowchart LR
  ...
  FE --> Gateway[Agent 网关<br/>认证 / 鉴权 / 审计 / 上下文透传]
  ...
  Router --> PrepareAgent[数据准备专家 Agent]
  Router --> DevAgent[数据开发专家 Agent]
  Router --> OpsAgent[数据运维专家 Agent]
  Router --> GovernanceAgent[数据治理专家 Agent]
  Router --> SecurityAgent[数据安全专家 Agent]

  PrepareAgent --> ToolGateway[Agent 网关<br/>鉴权 / 幂等 / 参数校验 / 错误码]
  DevAgent --> ToolGateway
  OpsAgent --> ToolGateway
  GovernanceAgent --> ToolGateway
  SecurityAgent --> ToolGateway
  ...
  API -. trace .-> Obs
  Gateway -. trace .-> Obs
  PrepareAgent -. trace .-> Obs
  DevAgent -. trace .-> Obs
  OpsAgent -. trace .-> Obs
  GovernanceAgent -. trace .-> Obs
  SecurityAgent -. trace .-> Obs
```
...
flowchart TB
  ...
  subgraph L4[业务专家 Agent 层]
    A1[数据准备专家 Agent<br/>数据源 / 查表查列 / 指标 / 建模 / 血缘]
    A2[数据开发专家 Agent<br/>配置稽核 / 查看详情]
    A3[数据运维专家 Agent<br/>状态 / 重跑 / 终止 / DAG]
    A4[数据治理专家 Agent<br/>AI 元数据治理 / 标准词根 / 存储治理]
    A5[数据安全专家 Agent<br/>安全扫描 / 权限申请]
  end
...
sequenceDiagram
  ...
  participant API as FastAPI AI 助手服务
  participant R as LangGraph 主编排
  participant A as 数据准备专家 Agent
  participant M as 元数据管理平台接口
  participant G as 血缘图谱服务
  participant Risk as 数据安全专家 Agent
...
sequenceDiagram
  participant U as 用户
  participant FE as 助手前端
  participant GWY as Agent 网关
  participant API as FastAPI AI 助手服务
  participant R as LangGraph 主编排
  participant D as 数据准备专家 Agent
  participant GW as Agent 网关
...
flowchart LR
  ...
  Gateway[Agent 网关鉴权与审计] --> Trace
  FastAPI[FastAPI 请求处理] --> Trace
  Graph[LangGraph 节点执行] --> Trace
  Intent[意图识别结果] --> Trace
  Plan[计划生成结果] --> Trace
  AgentCall[专家 Agent 调用] --> Trace
...
4. 前端统一调用Agent 网关，FastAPI AI 助手服务不直接对前端暴露。
5. Agent 网关负责认证、鉴权、审计和用户/组织/角色上下文透传。
6. 工具调用统一经过 Agent 网关，不能让 Agent 直接访问业务系统。
