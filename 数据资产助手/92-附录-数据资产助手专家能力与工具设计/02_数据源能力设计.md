...
数据源能力负责数据源登记、连接检测、元数据采集任务发起和状态跟踪。它主要承接“新增数据源、检测数据源、采集元数据、查看数据源状态”等场景。

在数据准备专家场景中，数据源能力是主流程能力。它负责把“数据源登记办结、元数据采集、数据地图同步、安全扫描”串成一个可跟踪任务，但敏感字段识别和安全等级判断由安全扫描能力完成。
...
| create_security_scan_task | 发起安全扫描 | 安全扫描能力 |
| query_security_scan_result | 查询扫描结果 | 安全扫描能力 |
...
flowchart TB
  ...
  NeedScan -- 是 --> SecurityAgent[调用安全扫描能力]
