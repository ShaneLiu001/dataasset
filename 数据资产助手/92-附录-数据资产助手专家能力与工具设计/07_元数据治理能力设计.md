...
元数据治理能力负责单表元数据治理流程的生成与发起，包括补充中文名、补充备注、填写安全等级等治理动作。它通常不是独立完成全部上下文采集，而是由数据资产助手编排层先调用数据地图能力完成同名表消歧，再收集用户的中台空间、项目、开发账号信息，然后调度数据标准、数据血缘、安全扫描等辅助能力获取治理上下文，最后由元数据治理能力生成治理流程。
...
| 工具 | 用途 | 数据来源 |
| --- | --- | --- |
| search_same_name_tables | 查询同名表候选 | 数据地图能力 / Elasticsearch |
| get_metadata_detail | 查询选定表当前元数据 | 元数据接口 |
| get_user_common_dev_context | 查询用户常用空间、项目、开发账号 | 用户偏好 / 会话记忆 |
| search_standards | 查询数据标准 | 数据标准能力 |
| query_upstream_lineage | 查询当前表上游血缘 | 数据血缘能力 |
| scan_security_level | 自动扫描安全等级 | 安全扫描能力 |
...
| 辅助能力 | 提供内容 | 用途 |
| --- | --- | --- |
| 数据地图能力 | 同名表候选、数据源 ID、schema、表名、字段、负责人、主题域、资产状态 | 表名消歧、用户二次确认唯一表、读取当前元数据 |
| 数据标准能力 | 候选标准、码值标准、标准定义、适用范围 | 生成字段标准映射和备注依据 |
| 数据血缘能力 | 当前表上游血缘、来源表字段、加工逻辑 | 补充字段来源语境和治理证据 |
| 安全扫描能力 | 后台自动扫描出的安全等级、敏感类型、脱敏策略 | 自动填写安全等级信息 |

## 7. 执行流程

```mermaid
flowchart TB
  Start[用户输入表名并发起治理] --> Search[调用数据地图能力查询同名表]
  ...
  ContextConfirm --> Standard[调用数据标准能力获取数据标准]
  ContextConfirm --> Lineage[调用数据血缘能力获取上游血缘]
  ContextConfirm --> Security[调用安全扫描能力后台自动扫描安全等级]
  Metadata --> Merge
  Standard --> Merge
  Lineage --> Merge
  Security --> Merge
  Merge --> Governance[调用元数据治理能力]
