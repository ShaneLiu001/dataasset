...
| 上下文管理 | Context Builder + Context Store + Context Policy | 统一组织用户、页面、资产、会话、任务、证据、记忆和权限上下文，负责裁剪、脱敏和传递 |
...
2. LangGraph 不直接拼接全量上下文，而是消费 Context Builder 输出的上下文包。
3. Tool Adapter 只接收调用接口必要的上下文字段，不把完整会话记忆传给每个 Tool。
4. LLM 只接收脱敏和裁剪后的证据摘要，不接收 token、连接串、密码、敏感样例值。
5. 调度系统恢复长流程时，先通过 task_id 恢复任务上下文，再触发助手服务继续执行。
