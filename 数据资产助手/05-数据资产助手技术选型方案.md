...
现有数据资产管理平台能力
  -> 新建 Agent 网关统一入口
  -> FastAPI AI 助手服务
  -> 上下文管理层组织用户、资产、任务、证据、知识库召回和记忆上下文
  -> LangGraph 多主控模式编排
  -> Tool Adapter 调用现有 Java API
  -> Activiti 流程引擎承接治理类任务
  -> XXL-JOB / 现有调度平台承接异步轮询、补偿和超时提醒
...
| Agent 编排 | LangGraph | 用于 Prompt Router、Control Mode Router、专家 Agent 调度、状态流转和人工确认节点，不建设传统细粒度意图识别体系 |
| 主控模式 | 多主控模式架构 | 根据 Prompt 类型选择 Skill 问答、单次 Tool、固定状态机、Agentic Loop、流式生成、调度任务、人工确认或专家 Agent 转交 |
| 统一知识库 | 已建设统一知识库，作为数小智和数据资产助手共同调用的知识来源 | 承载制度规范、操作手册、治理口径、FAQ、历史经验和可复用解释材料 |
| 上下文管理 | Context Builder + Context Store + Context Policy | 统一组织用户、页面、资产、会话、任务、证据、知识库召回、记忆和权限上下文，负责裁剪、脱敏和传递 |
| 服务框架 | Python + FastAPI | 作为 AI 助手服务入口，对接 Agent 网关和 LangGraph |
| 入口网关 | 新建 Agent 网关 | 负责认证、身份透传、审计、统一入口，不新建独立前端入口 |
...
| 调度系统 | XXL-JOB / 现有企业调度平台 | 承接异步轮询、失败补偿、超时提醒、长流程状态推进，不放在 LangGraph 内部硬跑 |
...
flowchart TB
  ...
  Web --> Gateway[Agent 网关<br/>认证 / 审计 / 身份透传]

  Gateway --> Assistant[FastAPI AI 助手服务]
  Assistant --> Context[上下文管理层<br/>Context Builder / Store / Policy<br/>汇聚 / 裁剪 / 脱敏 / 权限过滤]
  Context --> Graph[LangGraph 多主控编排层<br/>Prompt Router / Control Mode Router / Executor]
  Graph --> LLM[公司内网私有化大模型<br/>Function Calling / Tool Calling]

  Graph --> SkillCtl[Skill 问答<br/>规则说明 / 操作指引]
  Graph --> ToolCtl[单次 Tool 查询<br/>明确查询]
  Graph --> StateCtl[固定状态机<br/>办理 / 提交 / 创建]
  Graph --> LoopCtl[Agentic Loop<br/>诊断 / 分析 / 证据补齐]
  Graph --> StreamCtl[流式生成<br/>报告 / 总结 / 说明]
  Graph --> ConfirmCtl[人工确认<br/>写操作 / 高风险动作]

  Context --> Knowledge[统一知识库<br/>制度 / 规则 / 操作指引 / FAQ / 历史经验]
  Context --> Memory[助手侧记忆<br/>常用空间 / 项目 / 开发账号]
  Context --> RuntimeState[运行态状态<br/>Redis]
  Context --> TaskState[任务态 / 审计态<br/>GoldenDB]
  Context --> ES[Elasticsearch<br/>数据地图搜索]

  Graph --> ToolLayer[Tool Adapter 层<br/>参数校验 / API 调用 / 结果规范化]
  ToolCtl --> ToolLayer
  StateCtl --> ToolLayer
  LoopCtl --> ToolLayer
  ConfirmCtl --> ToolLayer
  ToolLayer --> JavaAPI[现有 Java API<br/>元数据 / 血缘 / 标准 / 安全扫描 / 稽核]
  ToolLayer --> ES
  ToolLayer --> Activiti[现有 Activiti 流程引擎<br/>治理流程 / 审批流程]
  ToolLayer --> Scheduler[XXL-JOB / 现有调度平台<br/>异步轮询 / 失败补偿 / 超时提醒]
  Graph --> SchedulerCtl[调度任务控制<br/>批量 / 定时 / 异步等待]
...
数据资产助手不采用传统的细粒度意图识别体系，而采用 `Prompt Router + Control Mode Router` 的多主控模式。技术上由 LangGraph 承载主图和子图，由 FastAPI 承载具体 Controller，由 Tool Adapter / 调度平台 / Activiti / 可观测平台承接确定性执行能力。

| Prompt 类型 | 主控模式 | 技术承载 | 典型场景 |
| --- | --- | --- | --- |
| 操作指引 / 规则说明 | Skill 问答 | Skill 模板 + Response 生成 | 如何登记数据源、字段命名规范 |
| 明确查询 | 单次 Tool 查询 | LangGraph Tool 节点 + Tool Adapter | 查字段列表、查表负责人、查安全等级 |
| 办理 / 提交 / 创建 | 固定状态机 | LangGraph 子图 / 显式状态机 + Confirm 节点 | 登记数据源、提交治理流程、创建稽核规则 |
| 诊断 / 分析 / 排查 | Agentic Loop | LangGraph loop 子图 + 只读 Tool 白名单 | 治理诊断、失败排查、影响分析 |
| 报告 / 总结 / 说明生成 | 流式生成 | FastAPI Streaming / SSE + LLM | 生成治理报告、整理评审口径 |
| 批量 / 定时 / 异步等待 | 调度任务 | XXL-JOB / 现有调度平台 + agent_task | 批量体检、流程状态轮询、超时提醒 |
| 写操作 / 高风险动作 | 人工确认 | Confirm 节点 + GoldenDB 确认记录 + Activiti | 修改备注、提交审批、下线资产 |
| 跨领域专业问题 | 专家 Agent 转交 | Handoff 节点 + 业务专家 Agent | 权限问题、血缘问题、安全问题、指标口径 |

推荐实现原则：

1. `Prompt Router` 先判断用户 Prompt 属于说明、查询、办理、诊断、生成、调度、写操作还是跨领域转交，并抽取必要参数和风险等级。
2. `Control Mode Router` 再选择对应主控模式，而不是默认进入 Agentic Loop。
3. Agentic Loop 默认只允许调用只读 Tool，写操作必须退出 Loop 并进入人工确认或固定状态机。
4. 固定状态机用于流程确定、状态可枚举、必须审计的场景。
5. 调度系统用于会话外等待、轮询、补偿和提醒，不由 Loop 挂起等待。
6. 流式生成只解决“长答案展示体验”，不承担生产动作决策。

一句话判断：

```text
能写死步骤的走固定流程；必须看中间结果再决定的走 Loop；
只是说明或查询的走 Skill / Tool；需要等待外部系统的走调度；
涉及写操作和生产影响的必须进入人工确认。
```
...
5. 大模型只负责理解自然语言、选择候选动作、生成解释和报告；确定性流程、写操作、调度补偿仍由状态机、Tool Adapter 和调度平台控制。
...
调度系统需要单独纳入技术选型。LangGraph 适合承接会话内的 Agent 编排、状态流转、人工确认和 Tool 调用决策，但不适合直接承担生产级定时任务、长时间轮询、失败补偿和超时提醒。

推荐一期复用公司已有调度体系；如果没有统一约束，可沿用数据准备专家方案中的 XXL-JOB。

| 调度职责 | 推荐承载 | 说明 |
| --- | --- | --- |
| 长流程状态轮询 | XXL-JOB / 现有调度平台 | 查询 Activiti 流程、数据源登记、安全扫描、稽核任务等异步结果 |
| 失败补偿 | XXL-JOB / 现有调度平台 | 对超时、接口失败、回调丢失的任务做补偿推进 |
| 超时提醒 | XXL-JOB / 现有调度平台 + 通知服务 | 对待用户确认、待审批、待处理任务做提醒 |
| 批量任务触发 | XXL-JOB / 现有调度平台 | 后续支持批量元数据补全、批量安全扫描、批量稽核规则检测 |
| 会话内节点推进 | LangGraph | 用户在线交互、确认节点、专家 Agent 协同和工具调用 |
| 任务状态落库 | GoldenDB | 调度任务只推进状态，最终状态仍以业务库和 agent_task 为准 |

推荐边界：

1. LangGraph 负责“当前这次用户请求怎么拆、调哪个 Tool、是否需要确认”。
2. 调度系统负责“任务提交后是否完成、失败后是否补偿、超时后是否提醒”。
3. GoldenDB 的 `agent_task` / `agent_task_step` 作为两者之间的任务状态中心。
4. 调度系统不直接调用大模型，必要时只恢复任务上下文并触发助手服务继续执行。

### 5.4 上下文管理选型建议

上下文管理需要作为独立工程能力纳入一期设计，但不建议一开始新建重型平台。推荐先在 FastAPI AI 助手服务内实现轻量 `Context Builder + Context Store + Context Policy`，后续再抽象为数小智统一上下文服务。

| 模块 | 一期承载 | 职责 |
| --- | --- | --- |
| Context Builder | FastAPI 服务内模块 | 汇聚 Agent 网关、前端、Redis、GoldenDB、ES、Java API 和 Tool 结果 |
| Context Store | Redis + GoldenDB | Redis 存短期会话运行态，GoldenDB 存任务态、确认记录和长期记忆 |
| Context Policy | FastAPI 服务内策略配置 | 控制上下文裁剪、敏感字段脱敏、权限过滤、TTL 和模型可见范围 |
| Context Envelope | Pydantic Schema / JSON Schema | 统一传给 LangGraph、专家 Agent 和 Tool Adapter 的上下文结构 |
| Knowledge Retriever | 统一知识库调用能力 | 按任务类型召回制度、规则、操作指引、FAQ 和历史经验，并交由 Context Policy 控制可见范围 |

推荐原则：

1. 上下文管理层先做逻辑模块，不单独拆服务，避免 Demo 和一期试点被基础设施拖慢。
2. 统一知识库已经作为公共知识来源存在，数据资产助手不重复建设知识库，而是通过上下文管理层按任务召回、裁剪和引用。
3. LangGraph 不直接拼接全量上下文，而是消费 Context Builder 输出的上下文包。
4. Tool Adapter 只接收调用接口必要的上下文字段，不把完整会话记忆传给每个 Tool。
5. LLM 只接收脱敏和裁剪后的证据摘要，不接收 token、连接串、密码、敏感样例值。
6. 调度系统恢复长流程时，先通过 task_id 恢复任务上下文，再触发助手服务继续执行。
...
```text
Demo 阶段：
  使用 Mock Tool Adapter，验证 LangGraph 多主控编排和用户交互。
...
| Tool 名称 | 主要归属专家 | 主要用途 | Demo 实现 | 一期真实接口 |
| --- | --- | --- | --- | --- |
...
采用“调用 Agent 网关登录接口获取 token，然后透传 token 和用户上下文”的方案。
...
| trace_id | 串联 Agent 网关、FastAPI、Java API、可观测平台 |
| space_id | 中台空间 |
| project_id | 中台项目 |
| dev_account | 开发账号 |
...
数据资产平台前端
  ...
  -> 前端请求 Agent 网关的助手入口
  -> Agent 网关转发给 FastAPI AI 助手服务
  -> FastAPI Tool Adapter 调用现有 Java API 时透传 token
```

推荐原则：

1. FastAPI AI 助手服务不直接面向前端暴露，统一走 Agent 网关。
2. Agent 网关负责登录、认证、基础审计和请求转发。
3. FastAPI 只接收 Agent 网关转发的可信请求。
4. Tool Adapter 调用 Java API 时透传 token、user_id、trace_id。
5. token 过期时优先由 Agent 网关或前端重新获取，不建议助手服务保存用户明文密码。

### 7.4 待确认问题

1. Agent 网关登录接口返回的 token 类型、有效期和刷新机制。
2. Agent 网关转发到 FastAPI 时使用 Header、Cookie 还是请求体传递用户上下文。
3. 现有 Java API 是否全部支持透传 Agent 网关 token。
...
### 8.2.1 Prompt 与主控模式映射

| 用户 Prompt 示例 | 推荐主控模式 | 说明 |
| --- | --- | --- |
| 如何登记数据源 | Skill 问答 | 确定性操作指引，不需要 Loop |
| 查一下 dwd_order_detail 的字段列表 | 单次 Tool 查询 | 目标明确，调用数据地图 / 元数据 Tool |
| 帮我登记一个 MySQL 数据源 | 固定状态机 | 收集表单、校验参数、用户确认、提交审批 |
| 为什么登记完地图里查不到表 | Agentic Loop | 逐步排查登记、审批、连通性、采集、ES 同步 |
| 这张表能不能下线 | Agentic Loop | 需要查访问热度、血缘、报表依赖、指标引用 |
| 生成这张表的治理诊断报告 | 流式生成，可接 Loop 结果 | 先取证，再生成长文本报告 |
| 每天检查缺备注字段并提醒 | 调度任务 | 批量、定时、异步任务交给调度平台 |
| 帮我提交元数据治理流程 | 人工确认 + 固定状态机 | 写操作必须确认、落库和审计 |
...
先跑 Demo，使用 Mock 数据，预留真实接口。入口先使用 Agent 网关 Mock，模拟 Agent 网关注入用户身份、token、trace_id 和组织上下文。

### 10.2 Demo 目标

1. 验证 Agent 网关 Mock 到 FastAPI 的调用方式。
...
| 助手记忆 | 模拟常用空间、项目、开发账号 |
| Agent 网关 Mock | 模拟 user_id、token、trace_id、org_id |
...
2. Demo 技术架构图。
3. 数据流向图。
4. 专家能力与工具设计，详见附录：[92-附录-数据资产助手专家能力与工具设计](92-附录-数据资产助手专家能力与工具设计/README.md)。
5. Tool 清单和接口字段草案。
...
1. FastAPI 助手服务。
2. LangGraph 主编排。
3. Prompt Router 与 Control Mode Router。
4. Agent 网关 Mock。
5. 数据地图 Mock Tool。
6. 数据标准 Mock Tool。
7. 数据血缘 Mock Tool。
8. 安全扫描 Mock Tool。
9. Activiti Mock Tool。
10. 助手侧记忆 Mock。
11. OpenTelemetry / Langfuse 预埋。
...
5. 能根据 Prompt 类型区分 Skill 问答、单次 Tool、固定状态机、Agentic Loop 和流式生成的主控路径。
6. 每次会话能查到 trace_id 和关键 Tool 调用记录。
...
| 2 | 身份透传方案 | 调用 Agent 网关登录接口获取 token，然后透传 | 确认 token 返回字段、有效期、刷新机制和 Header 规范 |
...
| 7 | Demo 是否接真实 Agent 网关 | 先用 Agent 网关 Mock | Demo 跑通后再切换真实 Agent 网关联调 |
| 8 | 调度系统接入边界 | 建议复用 XXL-JOB / 现有企业调度平台 | 确认公司当前统一调度平台、任务注册规范、告警通知方式和补偿重试策略 |
| 9 | 主控模式路由规则 | 建议采用多主控模式架构 | 确认 Prompt 分类口径、各模式边界、Loop 只读工具白名单和写操作确认规则 |
...
1. Agent 网关如何把用户身份和 trace_id 传给 FastAPI。
