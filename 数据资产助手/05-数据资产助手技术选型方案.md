...
| Agent 编排 | LangGraph | 用于 Prompt Router、Control Mode Router、子 Agent 调度、状态流转和人工确认节点，不建设传统细粒度意图识别体系 |
| 主控模式 | 多主控模式架构 | 根据 Prompt 类型选择 Skill 问答、单次 Tool、固定状态机、Agentic Loop、流式生成、调度任务、人工确认或子 Agent 转交 |
...
| 跨领域专业问题 | 子 Agent 转交 | Handoff 节点 + 领域 SubAgent | 权限问题、血缘问题、安全问题、指标口径 |
...
| 会话内节点推进 | LangGraph | 用户在线交互、确认节点、子 Agent 协同和工具调用 |
...
| Context Envelope | Pydantic Schema / JSON Schema | 统一传给 LangGraph、子 Agent 和 Tool Adapter 的上下文结构 |
