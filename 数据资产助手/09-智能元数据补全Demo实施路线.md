...
数据治理专家 Agent 是面向数据治理场景的业务专家 Agent，智能元数据补全是它的一期核心能力之一。平台模块不再直接等同于 Agent，数据地图、标准、血缘、安全扫描等统一作为可编排能力或 Tool 被专家调用。

推荐分层：

```text
数据治理专家 Agent：业务负责人 / 编排者
智能元数据补全能力：一期重点建设能力
元数据查询能力：提供资产、表、字段、负责人、已有备注等上下文
数据标准与词根能力：提供标准术语、字段标准、命名规范和标准映射依据
安全扫描能力：提供敏感字段识别、安全等级推荐和定级依据
血缘查询能力：提供上下游使用场景、加工任务和影响范围
Tool Adapter：真正调用现有 Java API、ES、安全扫描接口、标准接口等平台能力
```

数据治理专家 Agent 不只包含智能元数据补全，还可以逐步扩展以下能力：
...
本次 Demo 不要求一次性实现完整数据治理专家 Agent，而是先落地其中最容易形成业务价值和演示闭环的能力：
...
```text
本次 Demo 是“数据治理专家首期能力：智能元数据补全”。
数据治理专家 Agent 负责组织元数据查询、数据标准、词根、血缘查询、安全扫描等平台能力协同工作。
...
```text
这次 Demo 演示数据治理专家 Agent 的首期能力：智能元数据补全。
它不是一个孤立入口，而是支撑单表元数据治理、资产盘点等入口的公共能力。
数据治理专家 Agent 会协同元数据查询、数据标准、词根、血缘查询、安全扫描等能力，为补全生成提供上下文和依据。
...
第二步，数据治理专家 Agent 接管并调用智能元数据补全能力。
第三步，系统协同元数据查询、数据标准、词根、血缘查询、安全扫描等能力获取上下文。
...
```text
这个 Demo 证明了数据资产助手不仅能回答问题，还能通过数据治理专家 Agent 沉淀可被多个入口复用的元数据补全能力。
