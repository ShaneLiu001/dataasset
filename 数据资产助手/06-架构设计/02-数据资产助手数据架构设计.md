...
erDiagram
  ...
  AGENT_TASK ||--o{ AGENT_EVAL_RESULT : evaluated_by
  ...
  AGENT_EVAL_CASE ||--o{ AGENT_EVAL_RESULT : produces
  AGENT_EVAL_RESULT ||--o{ AGENT_EFFECT_FEEDBACK : receives
  ...
  AGENT_EVAL_CASE {
    varchar eval_case_id PK
    varchar scenario_code
    varchar question_text
    varchar expected_object_id
    varchar expected_answer_hash
    varchar label_source
    varchar case_status
  }

  AGENT_EVAL_RESULT {
    varchar eval_result_id PK
    varchar eval_case_id FK
    varchar task_id FK
  ...
  AGENT_EFFECT_FEEDBACK {
    varchar feedback_id PK
    varchar eval_result_id FK
    varchar task_id FK
...
可评测数据不替代业务审计数据，而是把任务执行结果、人工确认结果和标准样例关联起来，用于版本回归和效果复盘。

| 数据对象 | 说明 | 典型用途 |
| --- | --- | --- |
| agent_eval_case | 评测样例，保存标准问题、期望对象、期望答案摘要和标注来源 | 构造固定回归评测集 |
| agent_eval_result | 单次评测运行结果，关联模型版本、Prompt 版本、Tool 版本和任务 ID | 比较不同版本效果 |
| agent_effect_feedback | 用户采纳、修改、驳回、审批通过、审批退回等反馈摘要 | 计算采纳率、修改率、驳回率和高置信命中率 |

评测对象优先覆盖意图识别、查表推荐、元数据补全、安全等级推荐、血缘解释和稽核规则生成。生产环境只保存脱敏后的问题摘要、标准答案哈希、外部对象引用和反馈结果，不保存真实业务明细样本。
...
用于记录 LangGraph 节点、专家 Agent 执行、Tool 调用前后的阶段性状态。
...
| expert_code | varchar(64) |  | 专家 Agent 编码 |
...
sequenceDiagram
  ...
  participant E as agent_eval_result
...
6. 评测样例只保存脱敏问题、标准答案摘要、权威对象引用和人工标注来源。
...
| 评测样例 | 长期保留，按版本管理 |
| 评测运行结果 | 6 到 12 个月，关键版本结果可长期归档 |
| 效果反馈摘要 | 1 到 3 年，按审计和模型优化要求 |
...
| agent_eval_result.task_id | Langfuse / 评测平台 | 关联模型调用、评测样例和效果指标 |
| agent_effect_feedback.task_id | 用户确认 / Activiti | 关联采纳、修改、驳回和审批结果 |
