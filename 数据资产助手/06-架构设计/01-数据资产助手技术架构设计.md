...
V2 版本改为部署技术架构视角，重点表达浏览器、现有 Gateway、AI 助手服务、Agent 运行时、内网模型服务、现有 Java 服务、数据中间件和可观测平台分别部署在哪里、如何通信。
...
flowchart LR
  ...
  subgraph AssistantRuntime[AI 助手运行区]
    ...
    Graph[Agent 运行时<br/>LangGraph StateGraph<br/>Router / Planner / Executor / Confirm]
